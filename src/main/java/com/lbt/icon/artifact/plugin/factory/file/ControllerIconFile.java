 ...
 * @author johnson3yo
 */
public class ControllerIconFile implements IconFile {
        ...
        try {
            ...
            Writer writer = new FileWriter(new File(root.getAbsolutePath().
                    concat("/").concat(titles[0].substring(0, 1).toUpperCase().
                            concat(titles[0].substring(1)).concat("Controller").concat(".").concat("java"))));

            Velocity.mergeTemplate("/templates/controller.vm", "UTF-8", context, writer);
            writer.flush();
            writer.close();
