 ...
 * @author johnson3yo
 */
public class YamlConfigurationIconFile implements IconFile {
    ...
    public File generateFile(File root, String [] titles) throws IconArtifactException {
         try {
            ...
            Writer writer = new FileWriter(new File(root.getAbsolutePath().
                    concat("/").concat("application.yml")));

            Velocity.mergeTemplate("/templates/yamlconfig.vm", "UTF-8", context, writer);
            writer.flush();
            writer.close();
        ...
        throw new IconArtifactException("Could Not Create YAML Config File");
    }
