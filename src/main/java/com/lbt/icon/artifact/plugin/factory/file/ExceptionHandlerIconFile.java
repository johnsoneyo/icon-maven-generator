 ...
 * @author johnson3yo
 */
public class ExceptionHandlerIconFile implements IconFile{
    ...
    public File generateFile(File root, String[] titles) throws IconArtifactException {
        try {
            ...
            Writer writer = new FileWriter(new File(root.getAbsolutePath().
                    concat("/").concat("GlobalExceptionHandler".concat(".").concat("java"))));
            
            Velocity.mergeTemplate("/templates/globalexceptionhandler.vm", "UTF-8", context, writer);
            writer.flush();
            writer.close();
        ...
        throw new IconArtifactException("Could Not Create ExceptionHandler File");
    }
