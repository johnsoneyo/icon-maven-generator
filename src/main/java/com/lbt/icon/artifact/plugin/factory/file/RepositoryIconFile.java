 ...
 * @author johnson3yo
 */
public class RepositoryIconFile implements IconFile {
    ...
    public File generateFile(File root, String[] titles) throws IconArtifactException {
         try {
            ...
            Writer writer = new FileWriter(new File(root.getAbsolutePath().
                    concat("/").concat(titles[0].substring(0, 1).toUpperCase().
                            concat(titles[0].substring(1)).concat("Repository").concat(".").concat("java"))));

            Velocity.mergeTemplate("/templates/repository.vm", "UTF-8", context, writer);
            writer.flush();
            writer.close();
        ...
        throw new IconArtifactException("Could Not Create Boot Repository File");
    }
