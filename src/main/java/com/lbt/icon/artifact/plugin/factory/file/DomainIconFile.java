 ...
 * @author johnson3yo
 */
public class DomainIconFile implements IconFile {
    ...
    public File generateFile(File root, String[] titles) throws IconArtifactException {
        try {
            ...
            Writer writer = new FileWriter(new File(root.getAbsolutePath().
                    concat("/").concat(titles[0].substring(0, 1).toUpperCase().
                            concat(titles[0].substring(1)).concat(".").concat("java"))));

            Velocity.mergeTemplate("/templates/domain.vm", "UTF-8", context, writer);
            writer.flush();
            writer.close();
        ...
        throw new IconArtifactException("Could Not Create Boot Domain File");
    }
