 ...
 * @author johnson3yo
 */
public class SwaggerConfigIconFile implements IconFile {
    ...
    public File generateFile(File root, String[] titles) throws IconArtifactException {
        try {
            ...
            Writer writer = new FileWriter(new File(root.getAbsolutePath().
                    concat("/").
                            concat("SwaggerConfig").concat(".").concat("java")));

            Velocity.mergeTemplate("/templates/swaggerconfig.vm", "UTF-8", context, writer);
            writer.flush();
            writer.close();
        ...
        throw new IconArtifactException("Could Not Create Swagger Config File");
    }
